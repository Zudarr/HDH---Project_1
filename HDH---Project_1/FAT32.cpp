...
void FAT32::read(BYTE* sector)
{
	...
	reservedSectors = LE_read_reverse(sector, "E", 2); //offset E - 2 bytes
	fatCount = LE_read_reverse(sector, "10", 1); //offset 10 - 1 byte
	volumeSize = LE_read_reverse(sector, "20", 4); //offset 20 - 4 bytes
	fatSize = LE_read_reverse(sector, "24", 4); //offset 24 - 4 bytes
	startCluster = LE_read_reverse(sector, "2C", 4); //offset 2C - 4 bytes
	secondaryInfoSector = LE_read_reverse(sector, "30", 2); //offset 30 - 2 bytes
	bootCopySector = LE_read_reverse(sector, "32", 2); //offset 32 - 2 bytes
}
...
vector<int> clusterArray(FAT32 volume, int startCluster)
{
	...
	while (cluster != 268435455)
	{
		...
		ss >> offsetHex;
		cluster = LE_read_reverse(FAT, offsetHex, 4);
	}
	return result;
}
...
void folderHandler(string fileName, vector<byte> entry, int tab, FAT32 volume, vector< TxtFile>& txtFiles)
{
	...
	int startCluster = LE_read_reverse(highWord, "0", 2) * 256 + LE_read_reverse(lowWord, "0", 2);
...
void fileHandler(string fileName, string extension, vector<byte> entry, int tab, FAT32 volume, vector<TxtFile>& txtFiles)
{
	...
	printInfoOfMainEntry(entry);
	cout << " | ";
	int size = LE_read_reverse(&entry[0], "1C", 4);
